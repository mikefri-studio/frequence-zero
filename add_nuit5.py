# add_nuit5.py — ajoute la Nuit 5 « La Fréquence Morte » + Sonia
import os, sys
...
NUIT5_JS = r'''/* ============ DONNÉES — NUIT 5 « LA FRÉQUENCE MORTE » ============ */
const NUIT5 = {
...
# --- 1. js/nuit5.js : crée le fichier ---
f = os.path.join(BASE, "js", "nuit5.js")
if os.path.exists(f):
    print("⏭️  js/nuit5.js existe déjà")
else:
    ecrire(f, NUIT5_JS)
    print("✅ js/nuit5.js créé")

# --- 2. index.html : charge nuit5.js avant game.js ---
f = os.path.join(BASE, "index.html")
html = lire(f)
if "nuit5.js" in html:
    print("⏭️  index.html déjà modifié pour nuit5")
else:
    cible = '<script src="js/game.js"></script>'
    if cible not in html:
        sys.exit("❌ Balise game.js introuvable dans index.html")
    ecrire(f, html.replace(cible, '<script src="js/nuit5.js"></script>\n  ' + cible, 1))
    print("✅ index.html mis à jour (script nuit5.js)")

# --- 3. game.js : tableau NUITS ---
f = os.path.join(BASE, "js", "game.js")
c = lire(f)
modif = False
if "NUIT5" not in c:
    # Gère le cas où NUIT4 est le dernier, ou s'il n'y a que jusqu'à NUIT3
    if "NUIT4" in c:
        cible = "const NUITS = [NUIT1, NUIT2, NUIT3, NUIT4];"
        c = c.replace(cible, "const NUITS = [NUIT1, NUIT2, NUIT3, NUIT4, NUIT5];", 1)
    else:
        cible = "const NUITS = [NUIT1, NUIT2, NUIT3];"
        c = c.replace(cible, "const NUITS = [NUIT1, NUIT2, NUIT3, NUIT5];", 1)
    modif = True

if modif:
    ecrire(f, c)
    print("✅ js/game.js mis à jour (tableau NUITS)")
else:
    print("⏭️  js/game.js déjà à jour")

print("\n🌙 Nuit 5 intégrée avec succès !")
print('git add . && git commit -m "🌙 Ajout de la Nuit 5 : La Fréquence Morte" && git push')